...
class AdaINDCGANGenerator(nn.Module):
    def __init__(self, dim, h, w, out_features=16, z_size=128, condition_vector_size=0):
        ...
        self.block1 = nn.ConvTranspose2d(4 * dim, 2 * dim, 2, stride=2)
        self.ada1 = AdaIN(self.z_size + condition_vector_size, 2 * dim)
        self.block2 = nn.ConvTranspose2d(2 * dim, dim, 2, stride=2, padding=0)
        self.ada2 = AdaIN(self.z_size + condition_vector_size, dim)

        self.deconv_out = nn.ConvTranspose2d(dim, self.out_features, 2, stride=2, padding=0)
    ...
    def forward(self, input_tensor, cond=None):
        ...
        output = self.ada1(output, input_tensor)
        output = torch.relu(output)
        output = self.block2(output)  # x2 16,16
        output = self.ada2(output, input_tensor)
        output = torch.relu(output)
